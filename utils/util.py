...
def parse_args():
    ...
    parser.add_argument("--dataset", default='tdrive', type=str, help='dataset')
    ...
    parser.add_argument("--aspect", default=10, type=str, help='spatial or temporal (S or T)')
    parser.add_argument("--elements", default='road_segments', type=str, help='road_segments or points or hexes')
    return parser.parse_args()

def read_data(data, aspect, sim='TP', elems='road_segments', dtype='train'):
    sets = []
    aspects = {'S': 'roads', 'T': 'timelist', 'ST': 'stroads'}
    affix = aspects[aspect]
    if 'points' in elems or 'lat' in elems or 'lon' in elems: 
        affix = elems
    if elems == 'hexes':
        affix = 'hexes'
    with open('{}/{}/st_traj/{}/{}_{}_{}.txt'.format('../data', data, sim, data, affix, dtype), 'r') as f:
        num_items, num_sets = [int(x) for x in f.readline().strip().split('\t')]
        for line in f:
